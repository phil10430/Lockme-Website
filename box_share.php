...
if (preg_match('/^\d+$/', $boxId)) {
    $stmt = $pdo->prepare("
        ...
        WHERE ub.box_id = :box_id AND u.public_status = 1
    ");
...
if ($box) {
    ...
    $stmt = $pdo->prepare("
        SELECT name_top, name_sub, box_content, target_open_date
        FROM user_details
        WHERE box_id = :box_id
    ");
    $stmt->execute([':box_id' => $boxId]);
            ...
            <?php if ($details): ?>
                <div class="share-details">
                    <?php if (!empty($details['name_top'])): ?>
                        <span>Locker: <?= htmlspecialchars($details['name_top']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($details['name_sub'])): ?>
                        <span>Lockee: <?= htmlspecialchars($details['name_sub']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($details['target_open_date'])): ?>
                        <span>Target Open Date: <?= htmlspecialchars(date('d.m.Y H:i', strtotime($details['target_open_date']))) ?></span>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
