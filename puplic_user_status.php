...
// Registered boxes belonging to users who made their status public
$stmt = $pdo->prepare("
...
if (!empty($registeredBoxes)) {
    ...
    foreach ($historyRows as $r) {
        // first row we see per box is the newest, thanks to ORDER BY above
...
function box_state(?array $status): string
{
    ...
    if (in_array($val, ['1', 'locked', 'true', 'closed'], true)) {
        return 'locked';
    }
    if (in_array($val, ['0', 'unlocked', 'false', 'open'], true)) {
        return 'unlocked';
    }
    return 'unknown';
}
...
// Keep only boxes that are currently locked
...
// Load per-box details (Locker / Lockee / Content / Target Open Date) for the remaining boxes
$boxDetails = [];
...
<div class="status-wrapper">
    ...
    <div class="status-header">
        <h1>Locked Users</h1>
        <span class="status-count"><?= count($registeredBoxes) ?> locked</span>
    </div>

    <?php if (empty($registeredBoxes)): ?>
        <p class="status-empty">No locked boxes right now.</p>
    <?php else: ?>
        <div class="box-grid">
            ...
            ?>
            <div class="box-tile" data-state="locked" data-box-id="<?= htmlspecialchars($id) ?>">
